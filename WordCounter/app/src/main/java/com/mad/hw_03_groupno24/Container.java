...
import static com.mad.hw_03_groupno24.MainActivity.buffer;
...
public final class Container {
    ...
    public static Map<String, Integer> findAndCountWords(Map<String, Integer> list, boolean matchCase){
       ...
       // Map<String, Integer> counts = new HashMap<>();
        ...
        while (index < text.length()) {
            strings.add(new String(Arrays.copyOfRange(buffer, index, Math.min(index + 500,text.length()))));
            index += 500;
        }

        Message msg = new Message();
        msg.what=MainActivity.STATUS_START;
        Bundle data = new Bundle();    //this is like Extras, like a hashmap
        data.putInt("PROGRESS", strings.size());    //set key-value pairs
        msg.setData(data);
        ...
        for (String s: strings) {
            ...
            while (st.hasMoreTokens()) {
                ...
                if(!matchCase){
                    ...
                    for(String key : list.keySet()) {
                        if(null != key && key.equalsIgnoreCase(word)){
            ...
            msg = new Message();
            msg.what=MainActivity.STATUS_READ_WORDS;
            MainActivity.handler.sendMessage(msg);

        }
        Log.d("demo", "The array with counts: "+list);
        msg = new Message();
        msg.what=MainActivity.STATUS_DONE;
        MainActivity.handler.sendMessage(msg);
        return list;
    }
