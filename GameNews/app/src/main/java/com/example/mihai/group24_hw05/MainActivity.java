...
public class MainActivity extends AppCompatActivity implements SearchASYNC.GameData, GameDetailsAsync.GameDetails {
    ...
    public void setGameData(ArrayList<Game> list) {
        ...
        if(list!=null) {
            ...
            final RadioGroup rg = (RadioGroup) findViewById(R.id.radioResults);
            rg.removeAllViews();
            int i = 0;

            this.list.clear();
            for (Game choice : list) {
                final RadioButton rb = new RadioButton(this);
                this.list.put(i, choice);
                rb.setId(i);
                rb.setText(choice.getTitle() + ". " + "Released: " + choice.getDate() + ". Platform: " + choice.getPlatform()+ ".");
                i = i + 1;
                rb.setOnCheckedChangeListener(new CompoundButton.OnCheckedChangeListener() {
                    @Override
                    public void onCheckedChanged(CompoundButton buttonView, boolean isChecked) {
                        if (isChecked) {
                            findViewById(R.id.go_button).setClickable(true);
                        }else {
                            if(rg.getCheckedRadioButtonId()==-1) {
                                findViewById(R.id.go_button).setClickable(false);
                            }
                        }
                    }
                });
